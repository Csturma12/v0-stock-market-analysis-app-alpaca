...
export async function GET(
  ...
  try {
    const res = await fetch(`${BASE}/option-contract/${id}/intraday`, {
      headers: headers(),
      next: { revalidate: 60 },
    })
    ...
    const data = (json?.data ?? []).map((d: any) => ({
      ...
      askVol: Number(d.ask_volume ?? d.ask_side_volume ?? 0),
      bidVol: Number(d.bid_volume ?? d.bid_side_volume ?? 0),
      midVol: Number(d.mid_volume ?? 0),
    }))
